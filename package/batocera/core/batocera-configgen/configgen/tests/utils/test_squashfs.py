...
def test_squashfs_rom_mount_fails(subprocess_call: Mock) -> None:
    ...
    with (
        pytest.raises(Exception, match='^unable to mount the file /path/to/rom-name.squashfs'),
...
def test_squashfs_rom_umount_fails(subprocess_call: Mock) -> None:
    ...
    with (
        pytest.raises(Exception, match='^unable to unmount the file /var/run/squashfs/rom-name'),
        squashfs_rom(input_rom),
    ):
        pass
