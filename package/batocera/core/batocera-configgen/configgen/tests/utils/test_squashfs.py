...
def test_squashfs_rom_context_raises(subprocess_call: Mock, snapshot: SnapshotAssertion) -> None:
    ...
    with pytest.raises(Exception, match='^test exception$'), squashfs_rom(input_rom):
...
def test_squashfs_rom_mount_fails(subprocess_call: Mock) -> None:
    ...
    with (
        pytest.raises(BatoceraException, match='^Unable to mount the file /path/to/rom-name.squashfs'),
...
def test_squashfs_rom_umount_fails(subprocess_call: Mock) -> None:
    ...
    with (
        pytest.raises(BatoceraException, match='^Unable to unmount the file /var/run/squashfs/rom-name'),
        squashfs_rom(input_rom),
    ):
        pass
