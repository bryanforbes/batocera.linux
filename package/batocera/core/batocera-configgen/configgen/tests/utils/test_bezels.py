...
def test_resize_image(
    ...
    if rgba:
        ...
        fs.add_real_file(
            Path(__file__).parent.parent / '__files__' / 'bezels' / 'dreamcast.png', target_path='/tmp/dreamcast.png'
        )
...
def test_tatoo_image(
    ...
    fs.add_real_directory(
        Path(__file__).parent.parent / '__files__' / 'controller-overlays',
...
def test_create_transparent_bezel(size: tuple[int, int], snapshot: SnapshotAssertion) -> None:
    ...
    assert Path('/tmp/test.png') == snapshot(extension_class=PNGImageSnapshotExtension)
