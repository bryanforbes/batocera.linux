...
@pytest.fixture
def get_bezel_infos(mocker: MockerFixture, request: pytest.FixtureRequest) -> Mock:
...
@pytest.fixture
def fast_image_size(mocker: MockerFixture, request: pytest.FixtureRequest) -> Mock:
    return mocker.patch(
...
@pytest.fixture
def get_mame_machine_size(
    mocker: MockerFixture, generator_cls: type[MameGenerator], request: pytest.FixtureRequest
) -> Mock:
...
class TestMameGenerator(GeneratorBaseTest):
    ...
    def guns_borders_size_name(self, mocker: MockerFixture) -> Mock:
        return mocker.patch(
            'configgen.controllersConfig.gunsBordersSizeName',
            side_effect=[
    ...
    def guns_border_ratio_type(self, mocker: MockerFixture) -> Mock:
        return mocker.patch(
            'configgen.controllersConfig.gunsBorderRatioType',
            side_effect=[
    ...
    @pytest.fixture
    def video_mode_get_screens_infos(self, mocker: MockerFixture, request: pytest.FixtureRequest) -> Mock:
        ...
        *_make_system_config_params(
            {
                ...
                'fm7': {
                    ...
                    'configs': [
                        ...
                        # TODO: this setting doesn't do anything
...
def test_get_mame_machine_size_listxml_fails(
    ...
    with pytest.raises(Exception, match='mame -listxml machine failed'):
...
""",
    ...
    with pytest.raises(Exception, match='display element not found'):
        generator_cls.getMameMachineSize('machine', Path('/tmp/dir'))
