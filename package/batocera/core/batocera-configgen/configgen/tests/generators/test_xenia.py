...
def wine_reset() -> Iterator[None]:
    yield None

    wine.set_wine_runner(wine.DEFAULT_WINE_RUNNER)
...
class TestXeniaGenerator(GeneratorBaseTest):
    ...
    @pytest.fixture(autouse=True)
    def os_environ_nvidia(self, mocker: MockerFixture, request: pytest.FixtureRequest) -> None:
