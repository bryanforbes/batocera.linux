...
class TestDhewm3Generator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert Dhewm3Generator().getHotkeysContext() == snapshot

    def test_get_in_game_ratio(self) -> None:
        assert Dhewm3Generator().getInGameRatio({}, {'width': 0, 'height': 0}, '') == 16 / 9

    def test_generate(
        self, mock_system: Emulator, one_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        command = Dhewm3Generator().generate(
            mock_system,
...
seta r_brightness "1.6"
seta sys_lang "english"
""",
        )

        Dhewm3Generator().generate(
            mock_system,
            '/userdata/roms/doom3/rom.d3',
            one_player_controllers,
            {},
