...
class TestPcsx2Generator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert Pcsx2Generator().getHotkeysContext() == snapshot
    ...
    def test_get_in_game_ratio(self, mock_system_config: dict[str, Any], resolution: Resolution, result: bool) -> None:
        assert Pcsx2Generator().getInGameRatio(mock_system_config, resolution, '/userdata/roms/ps2/rom.chd') == result

    def test_generate(
        self, mock_system: Emulator, one_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        assert (
            Pcsx2Generator().generate(
                mock_system,
...
""",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param({}, {}, {1: {}}, id='1 Gun'),
            pytest.param({'controllers.pedals1': 'z'}, {}, {1: {}}, id='1 Gun, with pedal'),
            pytest.param({}, {}, {1: {}, 2: {}}, id='2 Guns'),
            pytest.param(
            ...
            pytest.param({}, {'gun_gun1port': '2'}, {1: {}}, id='1 Gun, Gun 1 on port 2'),
        ],
    )
    ...
    def test_generate_wheels(
        ...
        Pcsx2Generator().generate(
            mock_system,
            '/userdata/roms/ps2/rom.chd',
            {1: generic_xbox_pad_p1, 2: ps3_controller_p2},
            metadata,
            {},
            wheels,
            {},  # pyright: ignore
    ...
    def test_generate_multitap(
        self, mock_system: Emulator, generic_xbox_pad: Controller, num_controllers: int, snapshot: SnapshotAssertion
    ) -> None:
        Pcsx2Generator().generate(
            mock_system,
            '/userdata/roms/ps2/rom.chd',
            {n: make_player_controller(generic_xbox_pad, n) for n in range(1, num_controllers + 1)},
            {},
            {},
            {},
            {},  # pyright: ignore
    ...
    @pytest.mark.parametrize(
        ('status', 'response', 'mock_system_config'),
        [
            pytest.param(200, b'{ "Success": true, "Token": "RATOKEN" }', {}, id='success'),
            pytest.param(
                200,
                b'{ "Success": true, "Token": "RATOKEN" }',
                {
            ...
            pytest.param(200, b'{ "Success": false }', {}, id='failed'),
        ],
    )
    def test_generate_retroachievements(
        ...
        mock_http_instance = mocker.Mock()
        mock_http_instance.request.return_value = (mock_res, response)
        mocker.patch('httplib2.Http', return_value=mock_http_instance)
        ...
        assert (CONFIGS / 'PCSX2' / 'inis' / 'PCSX2.ini').read_text() == snapshot(name='ini')
        assert mock_http_instance.request.call_args_list == snapshot(name='request')
