...
class TestRpcs3Generator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert Rpcs3Generator().getHotkeysContext() == snapshot

    def test_get_in_game_ratio(self, snapshot: SnapshotAssertion) -> None:
        assert Rpcs3Generator().getInGameRatio({}, {'width': 1, 'height': 1}, '') == snapshot

    def test_generate(
        self, mock_system: Emulator, one_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        assert (
            Rpcs3Generator().generate(
                mock_system,
        ...
        assert (CONFIGS / 'rpcs3' / 'input_configs' / 'gamecontrollerdb.txt').read_text() == snapshot(
            name='controllerdb'
        )
...
""",
    ...
    def test_generate_controllers(
        ...
        controllers = {
            ...
            3: make_player_controller(generic_xbox_pad, 3),
            4: make_player_controller(generic_xbox_pad, 4),
            5: make_player_controller(ps3_controller, 5),
            6: make_player_controller(ds4_controller, 6),
            7: make_player_controller(ds5_controller, 7),
            8: make_player_controller(generic_xbox_pad, 8),
        }
        controllers[2].real_name = controllers[1].real_name
        Rpcs3Generator().generate(
            mock_system,
            '/userdata/roms/ps3/rom_dir',
            controllers,
