...
class TestLindberghGenerator(GeneratorBaseTest):
    ...
    @pytest.fixture
    def download_response(self, mocker: MockerFixture) -> Mock:
        ...
        def side_effect(chunk_size: int = -1) -> Iterator[bytes]:
            while chunk := tar_content.read(chunk_size):
                yield chunk
    ...
    @pytest.fixture
    def requests_get(self, requests: Mock, download_response: Mock) -> Mock:
        requests.get.return_value = download_response
        return requests.get

    @pytest.fixture(autouse=True)
    def subprocess_run(self, mocker: MockerFixture, subprocess_run: Mock, request: pytest.FixtureRequest) -> Mock:
        value = getattr(request, 'param', '')

        if isinstance(value, Exception):
            subprocess_run.side_effect = value
        else:
            subprocess_run.return_value = mocker.Mock(stdout=value)
...
""",
    ...
    @pytest.mark.usefixtures('requests_get')
    def test_generate_download(self, fs: FakeFilesystem, generator: Generator, mock_system: Emulator) -> None:
    ...
    def test_generate_download_error_response(
        self, fs: FakeFilesystem, generator: Generator, download_response: Mock, mock_system: Emulator
    ) -> None:
    ...
    def test_generate_tennis(
        ...
        assert filecmp.cmp(ROMS / 'lindbergh' / 'Virtua Tennis 3' / 'libCg.so', '/lib32/extralibs/libCg.so.tennis')
        assert filecmp.cmp(ROMS / 'lindbergh' / 'Virtua Tennis 3' / 'libCgGL.so', '/lib32/extralibs/libCgGL.so.tennis')
    ...
    def test_generate_tennis_existing(
        ...
        assert not filecmp.cmp(ROMS / 'lindbergh' / 'Virtua Tennis 3' / 'libCg.so', '/lib32/extralibs/libCg.so.tennis')
        assert not filecmp.cmp(
            ROMS / 'lindbergh' / 'Virtua Tennis 3' / 'libCgGL.so', '/lib32/extralibs/libCgGL.so.tennis'
        )
    ...
    @pytest.mark.parametrize('rom_name', ['stage 4', 'initiad4'])
    ...
    @pytest.mark.parametrize('speed_value', [None, '0', '1'])
    @pytest.mark.parametrize(
        'subprocess_run',
        ['', 'Current Speed:   12345  MHz', subprocess.CalledProcessError(-1, [])],
        indirect=True,
        ids=['none', '12345', 'exception'],
    )
    @pytest.mark.parametrize('rom_name', ['hotd4', 'hotd4sp', 'hotdex'])
    def test_generate_hotd_cpu_speed(
        ...
        assert (ROMS / 'lindbergh' / 'rom_dir' / 'lindbergh.conf').read_text() == snapshot(name='config')
        assert subprocess_run.call_args_list == snapshot(name='get_cpu_speed')
