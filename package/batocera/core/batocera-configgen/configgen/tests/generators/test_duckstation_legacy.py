...
from configgen.generators.duckstation_legacy.duckstationLegacyGenerator import DuckstationLegacyGenerator
...
class TestDuckstationLegacyGenerator:
    @pytest.fixture
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert DuckstationLegacyGenerator().getHotkeysContext() == snapshot

    def test_generate(
        self, mock_system: Emulator, one_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        command = DuckstationLegacyGenerator().generate(
            mock_system,
...
""",
        ...
        DuckstationLegacyGenerator().generate(
            mock_system,
            '/userdata/roms/psx/rom.chd',
            one_player_controllers,
            {},
...
""",
    ...
    def test_generate_no_bios_raises(
        ...
        generator = DuckstationLegacyGenerator()
    ...
    @pytest.mark.parametrize('bios_filename', ['scph1001.bin', 'scph5502.bin', 'scph3000.bin'])
    ...
    def test_generate_controllers(
        ...
        DuckstationLegacyGenerator().generate(
            mock_system,
            '/userdata/roms/psx/rom.chd',
            {n: make_player_controller(generic_xbox_pad, n) for n in range(1, controller_count + 1)},
            {},
            {},
            {},
            {'width': 1920, 'height': 1080},
    ...
    def test_generate_guns(
        ...
        DuckstationLegacyGenerator().generate(
            mock_system,
            '/userdata/roms/psx/rom.chd',
            {n: make_player_controller(generic_xbox_pad, n) for n in range(1, 6)},
            metadata,
            {'1': object()},  # pyright: ignore
            {},
            {'width': 1920, 'height': 1080},
        )
        assert (CONFIGS / 'duckstation' / 'settings.ini').read_text() == snapshot(name='settings')
