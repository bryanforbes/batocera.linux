...
""",
    ...
    @pytest.fixture(autouse=True)
    def ffmpeg_probe(self, mocker: MockerFixture, request: pytest.FixtureRequest) -> Mock:
    ...
    @pytest.fixture(autouse=True)
    def guns_borders_size_name(self, mocker: MockerFixture, request: pytest.FixtureRequest) -> Mock:
        return mocker.patch(
            'configgen.controllersConfig.gunsBordersSizeName', return_value=getattr(request, 'param', None)
        )
