...
from configgen.generators.cdogs.cdogsGenerator import CdogsGenerator
...
class TestCdogsGenerator:
    ...
    def fs(self, fs: FakeFilesystem) -> FakeFilesystem:
        ...
        for dir in assetdirs:
            fs.create_dir(ROMS / 'cdogs' / dir)

        return fs

    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert CdogsGenerator().getHotkeysContext() == snapshot

    @pytest.mark.usefixtures('fs')
    def test_generate(
        self, mocker: MockerFixture, one_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        command = CdogsGenerator().generate(
            mocker.Mock(),
            '',
