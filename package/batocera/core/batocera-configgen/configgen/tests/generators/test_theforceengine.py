...
class TestTheForceEngineGenerator:
    @pytest.fixture
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert TheForceEngineGenerator().getHotkeysContext() == snapshot

    def test_get_mouse_mode(self) -> None:
        assert TheForceEngineGenerator().getMouseMode({}, '')
    ...
    def test_get_in_game_ratio(self, mock_system_config: dict[str, Any], result: bool) -> None:
        assert TheForceEngineGenerator().getInGameRatio(mock_system_config, {'width': 0, 'height': 0}, '') == result
    ...
    def test_generate(
        ...
        assert (
            TheForceEngineGenerator().generate(
                mock_system,
...
""",
    ...
    def test_generate_tfe_file(
        ...
        fs.create_file(CONFIGS / 'theforceengine' / 'Mods' / 'v3.zip')
        fs.create_file(ROMS / 'theforceengine' / 'rom.tfe', contents='v4.zip')
