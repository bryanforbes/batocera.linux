...
class TestAmiberryGenerator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        ...
        assert AmiberryGenerator().getHotkeysContext() == snapshot
    ...
    def test_generate_lha(
        ...
        command = AmiberryGenerator().generate(
            mock_system,
        ...
        assert Path(CONFIGS / 'amiberry' / 'conf' / 'retroarch' / 'inputs' / 'real name 1.cfg').read_text() == snapshot(
            name='real name 1.cfg'
        )
    ...
    def test_generate_two_controllers(
        self, mock_system: Emulator, two_player_controllers: ControllerMapping, snapshot: SnapshotAssertion
    ) -> None:
        from configgen.generators.amiberry.amiberryGenerator import AmiberryGenerator
        ...
        assert Path(CONFIGS / 'amiberry' / 'conf' / 'retroarch' / 'inputs' / 'real name 2.cfg').read_text() == snapshot(
            name='real name 2.cfg'
        )
