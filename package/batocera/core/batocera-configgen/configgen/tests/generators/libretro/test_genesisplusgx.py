...
@pytest.mark.core('genesisplusgx')
@pytest.mark.fallback_system_name('megadrive')
class TestLibretroGeneratorGenesisPlusGX(LibretroBaseCoreTest):
    @pytest.mark.parametrize_core_configs(
        ...
        {
            'megadrive': [
                {'gpgx_blargg_filter_md': 'composite'},
                {'gpgx_fm': 'nuked (ym2612)'},
                {'controller1_md': '1'},
                {'controller2_md': '257'},
            ],
            'segacd': [
                {'gpgx_cd_add_on': 'megasd'},
                {'gpgx_cdda_volume': '90'},
                {'gpgx_fm': 'nuked (ym2612)'},
            ],
            'msu-md': [
            ...
            'mastersystem': [
                {'gpgx_blargg_filter_ms': 'composite'},
                {'ym2413': ['automatic', 'disabled']},
                {'controller1_ms': '260'},
                {'controller2_ms': '261'},
            ],
            'gamegear': [
    ...
    @parametrize_guns(systems=['megadrive', 'mastersystem', 'segacd'], metadata=[{'gun_type': 'justifier'}])
    def test_generate_guns(
        self, generator, fs, default_extension, mock_system, metadata, controllers, snapshot
    ) -> None:
        return super().test_generate_guns(
            generator, fs, default_extension, mock_system, metadata, controllers, snapshot
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            *(
                ('megadrive', config)
                for config in get_configs_with_base({}, [('gun_cursor_md', ['disabled', 'enabled'])])
            ),
            *(
                ('mastersystem', config)
                for config in get_configs_with_base({}, [('gun_cursor_ms', ['disabled', 'enabled'])])
            ),
        ],
        ids=str,
    )
