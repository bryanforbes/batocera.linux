...
class TestLexaloffleGenerator(GeneratorBaseTest):
    ...
    def test_generate_raises_system_name(
        ...
        with pytest.raises(
            BatoceraException, match='^The Lexaloffle generator has been called for an unknwon system: something_else.$'
        ):
            generator.generate(
                mock_system,
                ROMS / 'voxatron' / 'rom.png',
    ...
    def test_generate_raises_no_bin_path(
        ...
        with pytest.raises(BatoceraException, match='^Lexaloffle official binary not found at '):
            generator.generate(
                mock_system,
