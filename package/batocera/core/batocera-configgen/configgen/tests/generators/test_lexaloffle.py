...
class TestLexaloffleGenerator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert LexaloffleGenerator().getHotkeysContext() == snapshot

    def test_get_in_game_ratio(self) -> None:
        assert LexaloffleGenerator().getInGameRatio({}, {'width': 0, 'height': 0}, '') == 4 / 3
