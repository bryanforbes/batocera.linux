...
class TestScummVMGenerator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert ScummVMGenerator().getHotkeysContext() == snapshot
    ...
    def test_get_in_game_ratio(self, mock_system_config: dict[str, Any], result: bool) -> None:
        assert ScummVMGenerator().getInGameRatio(mock_system_config, {'width': 0, 'height': 0}, '') == result
