...
class TestFallout1Generator:
    ...
    def test_get_hotkeys_context(self, snapshot: SnapshotAssertion) -> None:
        assert Fallout1Generator().getHotkeysContext() == snapshot

    def test_get_mouse_mode(self) -> None:
        assert Fallout1Generator().getMouseMode({}, '')

    def test_get_in_game_ratio(self) -> None:
        assert Fallout1Generator().getInGameRatio({}, {'width': 0, 'height': 0}, '') == 16 / 9
    ...
    def test_generate(
        ...
        command = Fallout1Generator().generate(
            mock_system,
            '',
