...
@pytest.mark.usefixtures('fs_with_profiler_file')
def test_profile_enabled_on_import(cprofile_profile: Mock) -> None:
...
class TestGetHudBezel:
    ...
    @pytest.fixture
    def bezels_fast_image_size(self, mocker: MockerFixture) -> Mock:
    ...
    def test_get(
        ...
        fs.add_real_directory(
            Path(__file__).parent / '__files__' / 'bezels',
            target_path='/userdata/mybezels',
        )
        ...
        assert hud_bezel.read_bytes() == snapshot(extension_class=PNGImageSnapshotExtension)
    ...
    def test_screen_and_bezel_ratio_do_not_match(
        ...
        if not info_exists or not has_size:
            bezels_fast_image_size.return_value = (1920, 1087)
...
def test_extract_game_infos_from_xml(fs: FakeFilesystem, line_to_omit: str | None, snapshot: SnapshotAssertion) -> None:
    fs.create_file(
        '/tmp/game.xml',
...
<gameList parentHash="0">
    ...
    from configgen.emulatorlauncher import extractGameInfosFromXml

    assert extractGameInfosFromXml('/tmp/game.xml') == snapshot


def test_extract_game_infos_from_xml_invalid_xml(fs: FakeFilesystem) -> None:
    fs.create_file('/tmp/game.xml')

    from configgen.emulatorlauncher import extractGameInfosFromXml

    assert extractGameInfosFromXml('/tmp/game.xml') == {}
...
def test_run_command_no_commands(subprocess_popen: Mock) -> None:
    ...
    assert runCommand(Command([], {'ENV_VAR_1': '1'})) == -1
...
def test_run_command_error_handling(mocker: MockerFixture, exception: Exception, subprocess_popen: Mock) -> None:
    ...
    assert runCommand(Command(['command'], {})) == -1
...
class TestStartRom:
    ...
    @pytest.fixture(autouse=True)
    def controllers_get_guns(self, mocker: MockerFixture) -> Mock:
        return mocker.patch('configgen.controllersConfig.getGuns', return_value=mocker.sentinel.get_guns)
    ...
    @pytest.fixture(autouse=True)
    def controllers_guns_borders_size_name(self, mocker: MockerFixture) -> Mock:
        return mocker.patch(
            'configgen.controllersConfig.gunsBordersSizeName', return_value=mocker.sentinel.guns_borders_size_name
        )

    @pytest.fixture(autouse=True)
    def controllers_guns_border_ratio_type(self, mocker: MockerFixture) -> Mock:
        return mocker.patch(
            'configgen.controllersConfig.gunsBorderRatioType', return_value=mocker.sentinel.guns_border_ratio_type
        )
    ...
    def emulator_class(self, mocker: MockerFixture, mock_system: MockEmulator) -> Mock:
        ...
        def emulator_side_effect(system_name: str, *args: Any, **kwargs: Any) -> MockEmulator:
    ...
    @pytest.fixture
    def guns_utils_precalibration(self, mocker: MockerFixture) -> Mock:
        return mocker.patch('configgen.utils.gunsUtils.precalibration')
    ...
    def test_call(
        ...
        guns_utils_precalibration: Mock,
        get_generator: Mock,
        generator: Mock,
        video_mode_change_mode: Mock,
        evmapy: Mock,
        set_hotkeygen_context: Mock,
        controllers_guns_borders_size_name: Mock,
        controllers_guns_border_ratio_type: Mock,
        ...
        assert mock_system.config == {
            **default_config,
            'sdlvsync': '1',
        }
        assert os.environ['SDL_RENDER_VSYNC'] == '1'

        emulator_class.assert_called_once_with('system_name', '/userdata/roms/foo/bar.squashfs')
        controllers_get_games_metadata.assert_called_once_with('system_name', '/var/squashfs/bar/bar.zip')
        guns_utils_precalibration.assert_not_called()
        ...
        set_hotkeygen_context.assert_called_once_with(generator)
        ...
        controllers_guns_borders_size_name.assert_called_once_with({}, mock_system.config)
        controllers_guns_border_ratio_type.assert_called_once_with({}, mock_system.config)
    ...
    def test_ra_password_not_in_log(
        self,
        mocker: MockerFixture,
        default_args: Namespace,
        mock_system: Emulator,
    ...
    @pytest.mark.usefixtures('fs_with_profiler_file')
    def test_with_profiler(self, mocker: MockerFixture, default_args: Namespace, cprofile_profile: Mock) -> None:
        from configgen.emulatorlauncher import start_rom
        ...
        assert cprofile_profile.enable.call_args_list == [
            mocker.call(),
            mocker.call(),
        ]
    ...
    def test_args_emulator(
        self, mocker: MockerFixture, default_args: Namespace, mock_system: Emulator, get_generator: Mock, evmapy: Mock
    ) -> None:
        from configgen.emulatorlauncher import start_rom
        ...
        default_args.emulator = 'cmd-line-emulator'
        ...
        assert mock_system.config['emulator-forced']

        get_generator.assert_called_once_with('cmd-line-emulator')

        assert call_external_scripts.call_args_list == [
            mocker.call(
                SYSTEM_SCRIPTS,
                'gameStart',
                ['system_name', 'cmd-line-emulator', 'default-core', '/var/squashfs/bar/bar.zip'],
            ),
            mocker.call(
                USER_SCRIPTS,
                'gameStart',
    ...
    def test_args_core(
        self, mocker: MockerFixture, default_args: Namespace, mock_system: Emulator, evmapy: Mock
    ) -> None:
        from configgen.emulatorlauncher import start_rom
        ...
        default_args.core = 'cmd-line-core'
        ...
        assert mock_system.config['core-forced']

        assert call_external_scripts.call_args_list == [
            mocker.call(
                SYSTEM_SCRIPTS,
                'gameStart',
                ['system_name', 'default-emulator', 'cmd-line-core', '/var/squashfs/bar/bar.zip'],
            ),
            mocker.call(
                USER_SCRIPTS,
                'gameStart',
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('netplaypass', 'netplay.password'),
            ('netplayip', 'netplay.server.ip'),
            ('netplayport', 'netplay.server.port'),
            ('netplaysession', 'netplay.server.session'),
    ...
    def test_args(
        ...
        setattr(default_args, arg, mocker.sentinel.arg_value)
        ...
        assert mock_system.config == {**default_config, 'sdlvsync': '1', config_key: mocker.sentinel.arg_value}
    ...
    @pytest.mark.parametrize('use_lightgun_arg', [False, True])
    def test_guns(
        self,
        mocker: MockerFixture,
        default_args: Namespace,
        ...
        guns_utils_precalibration.assert_called_once_with(
            'system_name', 'default-emulator', 'default-core', '/var/squashfs/bar/bar.zip'
        )
        evmapy.assert_called_once_with(
            'system_name',
        ...
        controllers_guns_borders_size_name.assert_called_once_with(mocker.sentinel.get_guns, mock_system.config)
        controllers_guns_border_ratio_type.assert_called_once_with(mocker.sentinel.get_guns, mock_system.config)

    @pytest.mark.parametrize('empty_process_list', [False, True])
    @pytest.mark.parametrize('use_wheel_arg', [False, True])
    def test_wheels(
        self,
        mocker: MockerFixture,
        default_args: Namespace,
    ...
    def test_hud_config(
        ...
        extract_game_infos = mocker.patch(
            'configgen.emulatorlauncher.extractGameInfosFromXml', return_value=mocker.sentinel.extracted_game_info
        )
        ...
        extract_game_infos.assert_called_once_with(mocker.sentinel.gameinfoxml)
