...
""",
    ...
    def test_init(self, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
...
""",
        ...
        emulator = Emulator('3ds', '/userdata/roms/3ds/rom#1=2.game')

        assert emulator.config == snapshot
        assert emulator.renderconfig == snapshot(name='renderconfig')
    ...
    def test_init_with_arch_defaults(self, snapshot: SnapshotAssertion) -> None:
        assert Emulator('pcengine', '').config == snapshot

    def test_init_with_arch_defaults_options(self, snapshot: SnapshotAssertion) -> None:
        assert Emulator('n64', '').config == snapshot

    def test_init_shaderset_system(self, snapshot: SnapshotAssertion) -> None:
        emulator = Emulator('gb', '')

        assert emulator.config == snapshot
        assert emulator.renderconfig == snapshot(name='renderconfig')

    @pytest.mark.parametrize('name', ['3ds', 'gb'])
    def test_init_shaderset_none(self, name: str, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents="""# Mock conf file
...
""",
        ...
        assert emulator.renderconfig == snapshot

    @pytest.mark.parametrize('name', ['3ds', 'gb'])
    def test_init_shaderset_user(self, name: str, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents="""# Mock conf file
...
""",
    ...
    def test_init_no_defaults_arch_files(self, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
    ...
    def test_init_no_defaults_arch_files_no_shaderset(self, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents="""# Mock conf file
global.shaderset=none
""",
    ...
    def test_init_no_default_in_defaults_file(self, snapshot: SnapshotAssertion) -> None:
...
""")
        ...
        emulator = Emulator('3ds', '')

        assert emulator.config == snapshot

    @pytest.mark.parametrize('section', ['global', 'system'])
    def test_init_forced_emulator(self, section: str, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents=f"""# Mock conf file
{'3ds' if section == 'system' else 'global'}.emulator=blah
""",
        )
        assert Emulator('3ds', '/userdata/roms/3ds/rom#1=2.game').config == snapshot

    @pytest.mark.parametrize('section', ['global', 'system'])
    def test_init_forced_core(self, section: str, fs: FakeFilesystem, snapshot: SnapshotAssertion) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents=f"""# Mock conf file
{'3ds' if section == 'system' else 'global'}.core=blah
""",
        )
        assert Emulator('3ds', '/userdata/roms/3ds/rom#1=2.game').config == snapshot
    ...
    def test_init_es_settings(
        self, draw_framerate: str | None, ui_mode: str | None, snapshot: SnapshotAssertion
    ) -> None:
...
""")
        ...
        assert Emulator('3ds', '').config == snapshot

    def test_init_es_settings_parse_error(self, snapshot: SnapshotAssertion) -> None:
        ES_SETTINGS.unlink()

        assert Emulator('3ds', '').config == snapshot

    def test_is_opt_set(self) -> None:
        emulator = Emulator('3ds', '')

        assert emulator.isOptSet('hud_support')
        assert not emulator.isOptSet('foo')

    def test_get_opt_boolean(self, fs: FakeFilesystem) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents="""# Mock conf file
global.zero=0
...
""",
    ...
    def test_get_opt_string(self, fs: FakeFilesystem) -> None:
        fs.create_file(
            BATOCERA_CONF,
            contents="""# Mock conf file
