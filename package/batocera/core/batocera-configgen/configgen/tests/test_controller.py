...
class TestController:
    ...
    def test_generate_sdl_game_db_line_hotkey_raises(self) -> None:
        ...
        with pytest.raises(BatoceraException, match="^Unknown controller input type: 'foo'"):
...
""",
    ...
    def test_load_for_players_no_files_raises(self) -> None:
        with pytest.raises(
            BatoceraException, match='^Could not find controller data for "Test Controller" with GUID "2"$'
        ):
            Controller.load_for_players(
                1,
                Namespace(
