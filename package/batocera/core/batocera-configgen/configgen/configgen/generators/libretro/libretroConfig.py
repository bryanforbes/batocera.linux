...
def writeLibretroConfig(generator: Generator, retroconfig: UnixSettings, system: Emulator, controllers: ControllerPlayerMapping, metadata: Mapping[str, str], guns: GunMapping, wheels: DeviceInfoMapping, rom: Path, bezel: str | None, shaderBezel: bool, gameResolution: Resolution, gfxBackend: str) -> None:
...
def createLibretroConfig(generator: Generator, system: Emulator, controllers: ControllerPlayerMapping, metadata: Mapping[str, str], guns: GunMapping, wheels: DeviceInfoMapping, rom: Path, bezel: str | None, shaderBezel: bool, gameResolution: Resolution, gfxBackend: str) -> dict[str, object]:
    ...
    if system.config['core'] in ['genesisplusgx', 'picodrive']:
        ...
        for i in range(1, min(5, len(controller_list) + 1)):
            ...
            if (pad.guid in valid_megadrive_controller_guids and pad.device_name in valid_megadrive_controller_names) or (system.isOptSet(f'{option}_controller{i}_mapping') and system.config[f'{option}_controller{i}_mapping'] != 'retropad'):
    ...
    if system.config['core'] in ['mupen64plus-next', 'parallel_n64']:
        ...
        for i in range(1, min(5, len(controller_list) + 1)):
            controller, pad = controller_list[i - 1]
            if (pad.guid in valid_n64_controller_guids and pad.device_name in valid_n64_controller_names) or (system.isOptSet(f'{option}-controller{i}') and system.config[f'{option}-controller{i}'] != 'retropad'):
