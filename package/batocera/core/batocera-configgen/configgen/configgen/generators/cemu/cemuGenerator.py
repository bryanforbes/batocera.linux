...
class CemuGenerator(Generator):
    ...
    def CemuConfig(configFile: Path, system: Emulator) -> None:
        ...
        # Language
        lang = system.config.get("cemu_console_language", "ui")
        if lang == "ui":
        ...
        # Vsync
        CemuGenerator.setSectionConfig(config, graphic_root, "VSync", system.config.get_str("cemu_vsync", "0")) # 0 = Off
        # Upscale Filter
        CemuGenerator.setSectionConfig(config, graphic_root, "UpscaleFilter", system.config.get_str("cemu_upscale", "2")) # 2 = Hermite
        # Downscale Filter
        CemuGenerator.setSectionConfig(config, graphic_root, "DownscaleFilter", system.config.get_str("cemu_downscale", "0")) # 0 = Bilinear
        # Aspect Ratio
        CemuGenerator.setSectionConfig(config, graphic_root, "FullscreenScaling", system.config.get_str("cemu_aspect", "0")) # 0 = Bilinear
