    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        try:
            ...
            for config_key, (json_key, generator_default_value) in json_settings_map.items():
                found_value = system.config.get(config_key)
                ...
                if found_value is not Config.MISSING:
                    try:
                        parsed_value = None
                        if target_type is bool:
                            ...
                            if isinstance(found_value, bool):
                                parsed_value = 1 if found_value else 0
                            else:
                                parsed_value = int(float(found_value))
                        elif target_type is float:
                             if isinstance(found_value, bool):
                                 parsed_value = 1.0 if found_value else 0.0
                             else:
                                 parsed_value = float(found_value)
                        ...
                        _logger.warning("Conversion failed for '%s' to %s. Using default '%s'. Error: %s", found_value, target_type.__name__, generator_default_value, e)
                        final_value = generator_default_value
