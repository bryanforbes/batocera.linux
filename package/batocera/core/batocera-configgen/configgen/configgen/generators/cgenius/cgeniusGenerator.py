...
class CGeniusGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        if "Video" not in config:
            config["Video"] = {}
        # aspect
        config["Video"]["aspect"] = system.config.get_str("cgenius_aspect", "4:3")
        # set false as we want the correct ratio
        config["Video"]["fullscreen"] = "false"
        config["Video"]["integerScaling"] = "false"
        # filter
        config["Video"]["filter"] = system.config.get_str("cgenius_filter", "none")
        # quality
        config["Video"]["OGLfilter"] = system.config.get_str("cgenius_quality", "nearest")
        # render resolution
        match system.config.get_str("cgenius_render"):
            case "240":
            ...
            case _:
                ...
                config["Video"]["gameWidth"] = "320"
        # mouse
        config["Video"]["ShowCursor"] = system.config.get_str("cgenius_cursor", "false")
