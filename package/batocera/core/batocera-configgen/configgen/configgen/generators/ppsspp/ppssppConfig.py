...
def createPPSSPPConfig(iniConfig: CaseSensitiveConfigParser, system: Emulator):
    ...
    # Graphics Backend
    iniConfig.set("Graphics", "GraphicsBackend", system.get_option("gfxbackend", "0 (OPENGL)"))
    # If Vulkan
    if system.get_option("gfxbackend") == "3 (VULKAN)":
    ...
    # Display FPS
    iniConfig.set("Graphics", "ShowFPSCounter", "3" if system.show_fps else "0") # 1 for Speed%, 2 for FPS, 3 for both

    # Frameskip
    iniConfig.set("Graphics", "FrameSkipType", "0") # Use number and not percent
    if (frameskip := system.get_option_str("frameskip", "automatic")) != 'automatic':
        iniConfig.set("Graphics", "FrameSkip", frameskip)
    else:
        iniConfig.set("Graphics", "FrameSkip", system.get_option_bool('rendering_mode', return_values=("0", "2")))

    # Buffered rendering
    if not system.get_option_bool('rendering_mode', True):
        ...
        # Internal Resolution
        iniConfig.set("Graphics", "InternalResolution", system.get_option_str("internal_resolution", "1"))
        # Auto frameskip
        iniConfig.set("Graphics", "AutoFrameSkip", str(system.get_option_bool("autoframeskip", True)))

    # VSync Interval
    iniConfig.set("Graphics", "VSyncInterval", str(system.get_option_bool('vsyncinterval', True)))

    # Texture Scaling Level
    iniConfig.set("Graphics", "TexScalingLevel", system.get_option("texture_scaling_level", "1"))
    # Texture Scaling Type
    iniConfig.set("Graphics", "TexScalingType", system.get_option("texture_scaling_type", "0"))
    # Texture Deposterize
    iniConfig.set("Graphics", "TexDeposterize", system.get_option("texture_deposterize", "True"))

    # Anisotropic Filtering
    iniConfig.set("Graphics", "AnisotropyLevel", system.get_option("anisotropic_filtering", "3"))
    # Texture Filtering
    iniConfig.set("Graphics", "TextureFiltering", system.get_option("texture_filtering", "1"))
   ...
   ## [SYSTEM PARAM]
    ...
    if system.get_option_bool('retroachievements'):
        iniConfig.set("SystemParam", "NickName", system.get_option('retroachievements.username', 'Batocera'))
    ...
    # Rewinding
    iniConfig.set("General", "RewindFlipFrequency", system.get_option_bool('rewind', return_values=("300", "0"))) # 300 = every 5 seconds
    # Cheats
    iniConfig.set("General", "EnableCheats", system.get_option("enable_cheats", "False"))
    # Don't check for a new version
    iniConfig.set("General", "CheckForNewVersion", "False")

    # SaveState
    iniConfig.set("General", "StateSlot", system.get_option_str("state_slot", "0"))
    ...
    for section_option, user_config_value in system.option_items(starts_with="ppsspp."):
