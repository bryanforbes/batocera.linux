    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        for pref_name, pref_attrs in preferences.items():
            ...
            for attr_name, attr_value in pref_attrs.items():
                ...
                if pref_name == 'ps2.limitframerate' and (vsync := system.get_option('play_vsync')):
                    pref_element.attrib['Value'] = vsync
                if pref_name == 'renderer.widescreen' and (widescreen := system.get_option('play_widescreen')):
                    pref_element.attrib['Value'] = widescreen
                if pref_name == 'system.language' and (language := system.get_option('play_language')):
                    pref_element.attrib['Value'] = language
                if pref_name == 'video.gshandler' and (api := system.get_option('play_api')):
                    pref_element.attrib['Value'] = api
                if pref_name == 'renderer.opengl.resfactor' and (scale := system.get_option('play_scale')):
                    pref_element.attrib['Value'] = scale
                if pref_name == 'renderer.presentationmode' and (mode := system.get_option('play_mode')):
                    pref_element.attrib['Value'] = mode
                if pref_name == 'renderer.opengl.forcebilineartextures' and (filter := system.get_option('play_filter')):
        ...
        def create_input_preferences(input_config: ET.Element, pad_guid: str, key_id: str | int, key_type: int, provider_id: int, nplayer: int, joystick_name: str, binding_type: str | int, hat_value: int):
