...
class RyujinxGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        try:
            with ryujinxConfFile.open("r") as f:
                conf = json.load(f)
        except Exception:
            conf = {}
        ...
        conf["system_language"] = system.get_option("ryujinx_language", "AmericanEnglish")
        conf["system_region"] = system.get_option("ryujinx_region", "USA")
        conf["system_time_zone"] = "UTC"
        conf["system_time_offset"] = system.get_option_int("ryujinx_timeoffset", 0)

        # Graphics
        conf["graphics_backend"] = system.get_option("ryujinx_api", "Vulkan")
        conf["res_scale"] = system.get_option_int("ryujinx_scale", 1)
        conf["aspect_ratio"] = system.get_option("ryujinx_ratio", "Fixed16x9")
        conf["max_anisotropy"] = system.get_option_int("ryujinx_filtering", -1)
