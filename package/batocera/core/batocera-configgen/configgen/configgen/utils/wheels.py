...
_EMULATOR_MAPPING: Final = {
    ...
    'wiiu': {'a': 'a', 'b': 'b', 'x': 'x', 'y': 'y', 'start': 'start', 'select': 'select'},
    'psx': {'cross': 'b', 'square': 'y', 'round': 'a', 'triangle': 'x', 'start': 'start', 'select': 'select'},
...
def configure_wheels(
        ...
        if pad.device_path in devices and devices[pad.device_path]['isWheel']:
            ...
            for md_key, md_value in metadata.items():
                if md_key.startswith('wheel_'):
                    ...
                    if (
                        (wheel_key := _WHEEL_MAPPING.get(short_md_key)) is not None
                        and md_value in _EMULATOR_MAPPING.get(system, {})
                        and wheel_key in pad.inputs
                    ):
            ...
            for md_key, md_value in metadata.items():
                if md_key.startswith('wheel_'):
                    short_md_key = md_key[6:]
                    if (wheel_key := _WHEEL_MAPPING.get(short_md_key)) is not None and (
                        wanted_key := _EMULATOR_MAPPING.get(system, {}).get(md_value)
                    ) is not None:
    ...
    for player_number, pad in sorted(controllers.items()):
        if (device := devices.get(pad.device_path)) is not None and device['isWheel'] and 'wheel_rotation' in device:
            ...
            wanted_ra = config.get_int('wheel_rotation', int(metadata.get('wheel_rotation', ra)))
            wanted_deadzone = config.get_int('wheel_deadzone', int(metadata.get('wheel_deadzone', 0)))
            wanted_midzone = config.get_int('wheel_midzone', int(metadata.get('wheel_midzone', 0)))
            ...
            if wanted_ra < ra or wanted_deadzone > 0:
                ...
                if newdev is not None:
                    _logger.info(
                        'replacing device %s by device %s for player %s', pad.device_path, newdev, player_number
                    )
    ...
    if recompute_sdl_ids:
        ...
        for pad in sorted(controllers.values()):
            if (
                pad.physical_device_path is not None
                and (device := devices.get(pad.physical_device_path)) is not None
                and 'joystick_index' in device
            ):
    ...
    try:
        yield controllers_new, {key: device_info for key, device_info in devices.items() if device_info['isWheel']}
    finally:
        try:
            for p in procs:
