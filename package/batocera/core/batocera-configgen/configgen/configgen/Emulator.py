...
class Emulator:
    ...
    def __post_init__(self, rom: str, /) -> None:
        ...
        controllersSettings = recalSettings.load_all('controllers', includeName=True)
        systemSettings = recalSettings.load_all(self.name)
        folderSettings = recalSettings.load_all(f'{self.name}.folder["{rom_path.parent}"]')
        gameSettings = recalSettings.load_all(f'{self.name}["{gsname}"]')
        ...
        systemSettings = recalSettings.load_all(f'{self.name}-renderer')
        gameSettings = recalSettings.load_all(f'{self.name}["{gsname}"]-renderer')
